...
PORT = int(os.environ.get("SEARCH_SERVICE_PORT", "8787"))
DEFAULT_LIMIT = 10
MAX_LIMIT = 20
MAX_CANDIDATES = 5
HTTP_TIMEOUT = 6.0
...
def _fetch_browser(
    ...
    """Fetch a rendered page and return its final URL and HTML."""
    ...
    def guard_route(route: Any) -> None:
        ...
        # Non-network browser resources such as data:, blob:, etc. are
        # allowed. Every HTTP(S) resource must pass the existing SSRF
        # validation before the browser is allowed to request it.
        if parsed.scheme.lower() in {"http", "https"}:
            if not is_url_safe(request_url):
                logger.warning(
                    "FETCH blocked unsafe resource url=%s",
                    request_url,
                )
...
def fetch_page(
    ...
    """Fetch a specific page with Camoufox and return Readability Markdown."""
    ...
    if browser_executor is None:
        return {
            "ok": False,
            "error": "browser_executor_not_running",
        }
    ...
    try:
        final_url, html_source = browser_executor.call(
            lambda browser: _fetch_browser(
                browser,
                url,
                int(
                    FETCH_TIMEOUT * 1000
        ...
        logger.info(
            "FETCH success url=%s final_url=%s title=%r chars=%d elapsed=%.3fs",
            url,
            final_url,
            title,
            len(content),
        ...
        return {
            ...
            "title": title,
            "content": content,
            "source": "browser_readability",
            "elapsed": elapsed,
        }

    except FutureTimeoutError:
        logger.warning(
            "FETCH timeout url=%s elapsed=%.3fs",
            url,
            time.perf_counter() - started,
        )
        ...
        logger.warning(
            "FETCH failed url=%s error=%s elapsed=%.3fs",
            url,
            type(exc).__name__,
            time.perf_counter() - started,
        )
...
class SearchHandler(
    ...
    server_version = "SearchService/7.1-fetch"
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if parsed.path == "/fetch":
                ...
                logger.info(
                    "FETCH REQUEST start id=%d url=%r",
                    request_id,
                    url,
                )
                ...
                if result.get("ok"):
                    status = 200
                elif result.get("error") == "unsafe_url":
                    status = 400
                else:
                    status = 502
                ...
                logger.info(
                    "FETCH REQUEST response id=%d ok=%s elapsed=%.3fs",
                    request_id,
                    result.get("ok"),
                    time.perf_counter() - started,
                )
